...
// HandleArticle acts on all http requests concerning articles
	...
	if len(p) == 3 && (req.Method == "POST" || ((req.Method == "GET" || req.Method == "PUT" || req.Method == "DELETE") && hasId)) {
		...
		case "GET":
			if id[0] != "*" {
				...
				qElem = &db.GetElement{Collection: "articles", Filter: bson.M{"_id": objID}, HTTPResponder: responder}
			} else {
				// get all elements
				qElem = &db.GetElement{Collection: "articles", Filter: bson.M{}, HTTPResponder: responder}
			}
			break
		case "POST":
			qElem = &db.InsertOneElement{Collection: "articles", Element: req.Body, Session: cookie.Value, HTTPResponder: responder}
			break
		case "PUT":
			objID, _ := primitive.ObjectIDFromHex(id[0])
			qElem = &db.ReplaceOneElement{Collection: "articles", Filter: bson.M{"_id": objID}, Element: req.Body, Session: cookie.Value, HTTPResponder: responder}
			break
		case "DELETE":
			objID, _ := primitive.ObjectIDFromHex(id[0])
			qElem = &db.DeleteOneElement{Collection: "articles", Filter: bson.M{"_id": objID}, Session: cookie.Value, HTTPResponder: responder}
			break
		}
